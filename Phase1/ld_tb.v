...
// Expected: R7 = 0x00000084 (preloaded at RAM[0x65] in datapath.v).
...
module ld_tb;
    ...
    // T1: Zlowout, PCin, Read          <- RAM reads memory[0]=LD_INSTR at posedge T1
    // T2: Read, MDRin                  <- MDR latches LD_INSTR
    // T3: MDRout, IRin                 <- IR naturally receives LD_INSTR
    // T4: Grb, BAout, Yin
    // T5: Cout, ADD, Zin
    // T6: Zlowout, MARin
    // T7: Read                         <- RAM reads memory[0x65] at posedge T7
    // T8: Read, MDRin                  <- MDR latches 0x84
    // T9: MDRout, Gra, Rin
    ...
    // Preload RAM[0] with ld instruction. RAM[0x65] is preloaded in datapath.v.
    initial begin
