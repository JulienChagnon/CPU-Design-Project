...
// addi R7, R4, -9 => R7 <- R4 + sign_extend(-9)
// Initialize: R4 = 0xF0
// Expected: R7 = 0xF0 + (-9) = 231 = 0x000000E7
//
// Better approach: preload RAM[0] with the instruction and use a correct
// 4-step fetch (T1=Read only, T2=Read+MDRin). Our RAM is synchronous —
// data_out updates at posedge when read=1, so MDRin in the same cycle as
// Read would latch OLD data. Splitting into two cycles lets IR load naturally
// without any force on IR_data_out.
...
module addi_tb;
    ...
    // addi R7, R4, -9: IR[26:23]=Ra=7, IR[22:19]=Rb=4, IR[18:0]=C=-9 (19'h7FFF7)
    // opcode bits[31:27] = 00011
    localparam [31:0] ADDI_INSTR = 32'h1BA7FFF7;

    // Control Sequence: addi (4-step fetch)
    // T0: PCout, MARin, IncPC, Zin
    // T1: Zlowout, PCin, Read              <- Read asserted; RAM loads at posedge T1
    // T2: Read, MDRin                      <- MDR latches ADDI_INSTR from RAM
    // T3: MDRout, IRin                     <- IR naturally receives ADDI_INSTR
    // T4: Grb, Rout, Yin
    // T5: Cout, ADD, Zin
    // T6: Zlowout, Gra, Rin
    ...
    // Preload RAM[0] with the addi instruction; set R4 after clear deasserts.
    initial begin
        DUT.mem_sys.ram_inst.memory[9'h000] = ADDI_INSTR;
        #25;
    ...
    always @(*) begin
        ...
        case (Present_state)
            ...
            T2: begin // Read, MDRin  (MDR latches instruction from RAM)
                Read = 1; MDRin = 1;
            end
