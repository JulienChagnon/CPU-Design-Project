...
// st 0x1F(R6), R6 => M[R6 + 0x1F] = M[0x63 + 0x1F] = M[0x82] <- R6
// Initialize: R6 = 0x63, RAM[0x82] = 0xA7 (preloaded in datapath.v).
// Expected: RAM[0x82] = 0x00000063 after store.
...
module st_tb2;
    ...
    // st 0x1F(R6), R6: IR[26:23]=Ra=6, IR[22:19]=Rb=6, IR[18:0]=C=0x1F
    // Effective address: R6 + 0x1F = 0x63 + 0x1F = 0x82
    localparam [31:0] ST_INSTR = 32'h1330001F;
    ...
    // T2: Read, MDRin
    // T3: MDRout, IRin
    // T4: Grb, BAout, Yin     (R6 onto bus; BAout only forces 0 when R0 selected)
    // T5: Cout, ADD, Zin
    // T6: Zlowout, MARin
    ...
    // R6 is both the base register (Rb) and the source data register (Ra).
    // RAM[0x82] is preloaded in datapath.v.
    initial begin
